...
switch ($filter) {
    case 'day':
        $dateFrom = $customDate; $dateTo = $customDate;
    ...
    case 'week':
        ...
        break;
    case 'year':
        $dateFrom = $customYear . '-01-01'; $dateTo = $customYear . '-12-31';
        $label = $customYear;
        break;
    case 'month':
    default:
...
$driversStmt = $pdo->prepare("
    ...
    LEFT JOIN schedules s ON s.driver_id = d.driver_id
        AND s.status = 'Completed' AND s.office_id = ?
        AND s.date_end BETWEEN ? AND ?
    WHERE (d.office_id = ? OR d.driver_scope = 'Both')
    GROUP BY d.driver_id ORDER BY d.driver_name ASC
");
...
$driverFilter = '';
if ($driverId > 0) { $driverFilter = ' AND s.driver_id = ?'; $params[] = $driverId; }

$tripsStmt = $pdo->prepare("
    SELECT s.*, s.trip_ticket_no,
           d.driver_name, d.license_no,
           v.brand, v.model, v.plate_number,
           u.username AS requestor_name,
           o.office_name, dep.dept_name,
           TIMESTAMPDIFF(HOUR,
    ...
    WHERE s.status = 'Completed' AND s.date_end BETWEEN ? AND ?
      AND s.office_id = ? {$driverFilter}
    ORDER BY s.date_end DESC, s.schedule_id DESC
");
$tripsStmt->execute($params);
$trips = $tripsStmt->fetchAll();
/* ── Resolve trip ticket numbers ── */
foreach ($trips as &$t) {
...
foreach ($trips as $t) {
    $did = $t['driver_id'];
    if (!isset($driverSummary[$did])) $driverSummary[$did] = ['name' => $t['driver_name'], 'count' => 0, 'hours' => 0];
...
if ($driverId > 0) {
    ...
    if (!$selectedDriverName && !empty($trips)) $selectedDriverName = $trips[0]['driver_name'];
}
...
function getTicketNo(PDO $pdo, int $id, string $ds, string $officeName): string {
    ...
    $pdo->prepare("
        UPDATE schedules SET trip_ticket_no = ? WHERE schedule_id = ?
    ")->execute([$ticketNo, $id]);
...
<title>Driver Completed Trips – CSU VSS Staff</title>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
<link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.0/font/bootstrap-icons.css" rel="stylesheet">
<style>
:root {
    --maroon: #800000; --maroon-dark: #5a0000; --maroon-light: #fdecea;
    --maroon-mid: #f0e5e5; --surface: #fff; --bg: #f5f0f0;
    --border: #eedede; --text: #1a1a1a; --muted: #888;
...
.staff-notice { background: #e8f4fd; border: 1.5px solid #bee3f8; border-radius: 10px; padding: .6rem 1rem; font-size: .78rem; color: #0c5480; display: flex; align-items: center; gap: 8px; margin-bottom: 1.1rem; }
...
@media (max-width: 768px) {
    ...
    .filter-period-tabs { width: 100%; }
    .driver-select { min-width: 100%; width: 100%; }
...
@media print {
    ...
    .sidebar,.topbar,.filter-card,.no-print,.stats-row,.driver-lb,.page-hdr,.period-pill,.staff-notice { display: none !important; }
    * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
    body { background: #fff !important; font-family: 'Segoe UI',sans-serif; font-size: 9pt; color: #000; padding: 12mm 14mm 14mm 14mm !important; }
    ...
    .table thead th { background: #fdecea !important; color: #800000 !important; border: 1px solid #e0c0c0 !important; padding: 5px 7px !important; font-size: 7pt !important; font-weight: 800 !important; white-space: nowrap; text-transform: uppercase; letter-spacing: .05em; display: table-cell !important; }
    .table tbody td { border: 1px solid #eedede !important; padding: 5px 7px !important; font-size: 8pt !important; color: #222 !important; vertical-align: middle !important; display: table-cell !important; }
    .table tbody tr:nth-child(even) td { background: #fdf8f8 !important; }
    .table tbody tr { page-break-inside: avoid; }
    .driver-chip { background: #fdecea !important; color: #800000 !important; font-size: 7pt !important; padding: 2px 6px !important; border-radius: 10px !important; }
    .dc-av { background: #800000 !important; color: #fff !important; width: 14px !important; height: 14px !important; font-size: 6pt !important; }
    ...
    .plate-chip { background: #f3f4f6 !important; color: #374151 !important; font-size: 6.5pt !important; padding: 1px 4px !important; }
    ...
    .ticket-chip i,.dest-cell i,.bi-calendar3 { display: none; }
...
<div class="sidebar">
  ...
  <nav class="nav flex-column mt-2">
    ...
    <a class="nav-link active" href="staff_driverstripcomplete.php"><i class="bi bi-flag-fill"></i> Driver Trip Records</a>
...
<div class="topbar">
  ...
  <div class="topbar-title">
    <i class="bi bi-flag-fill" style="color:var(--maroon)"></i> Driver Trip Records
  </div>
...
<div class="main-content">
  ...
  <div class="page-hdr no-print">
    <div>
      ...
      <p>View completed trips per driver — Office: <strong><?= htmlspecialchars($me['office_name'] ?? '—') ?></strong></p>
    </div>
    <span class="period-pill"><i class="bi bi-calendar3"></i> <?= htmlspecialchars($label) ?></span>
  </div>

  <div class="staff-notice no-print">
    <i class="bi bi-info-circle-fill"></i>
    <span>You are viewing completed trip records for your office. Records are read-only.</span>
  </div>
  ...
  <div class="filter-card no-print">
    <form method="GET" action="">
      <div class="filter-row">
        ...
        <a href="staff_driverstripcomplete.php"
  ...
  <strong>Date printed:</strong> <?=date('F j, Y g:i A')?>
</div>
  </div>

  <!-- Print summary -->
  <div class="print-summary">
    <div class="print-summary-item"><div class="ps-val"><?=$totalTrips?></div><div class="ps-lbl">Completed Trips</div></div>
    <div class="print-summary-item"><div class="ps-val"><?=fh($totalHours)?></div><div class="ps-lbl">Total Duration</div></div>
    <div class="print-summary-item"><div class="ps-val"><?=$uniqueDestinations?></div><div class="ps-lbl">Destinations</div></div>
  </div>
  ...
  <div class="section-card">
    <div class="section-hdr">
      <span>
        ...
        <?php if($driverId>0&&$selectedDriverName): ?>Trips by <strong><?=htmlspecialchars($selectedDriverName)?></strong>
        <?php else: ?>All Completed Trips — <?=htmlspecialchars($me['office_name']??'')?><?php endif;?>
        &mdash; <?=htmlspecialchars($label)?>
      </span>
      <span style="font-size:.78rem;font-weight:600;opacity:.85"><?=$totalTrips?> record<?=$totalTrips!==1?'s':''?></span>
    </div>
    <?php if(empty($trips)): ?>
    ...
    <div class="table-responsive">
      <table class="table mb-0">
        <thead><tr>
          <th>Request #</th><th>Trip Ticket #</th><th>Driver</th><th>Vehicle</th>
          <th>Destination</th><th>Purpose</th><th>Date</th><th>Time</th>
          <th>Duration</th><th>Requestor</th><th>Office</th>
        </tr></thead>
        <tbody>
          <?php foreach($trips as $t): $hours=max(0,(int)$t['duration_hours']); ?>
          <tr>
            <td style="font-weight:800;color:var(--maroon);white-space:nowrap">REQ-<?=str_pad($t['schedule_id'],4,'0',STR_PAD_LEFT)?></td>
            <td style="white-space:nowrap;">
              ...
              <?php else: ?><span style="color:#ccc;font-style:italic;font-size:.78rem">—</span><?php endif;?>
            </td>
            <td>
            ...
            <td>
              ...
              <?php else: ?><span style="color:#ccc;font-style:italic">N/A</span><?php endif;?>
            </td>
            <td class="dest-cell"><i class="bi bi-geo-alt-fill"></i> <?=htmlspecialchars($t['destination'])?></td>
            <td style="max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--muted);font-size:.8rem" title="<?=htmlspecialchars($t['purpose'])?>">
              <?=htmlspecialchars($t['purpose'])?>
            </td>
            <td style="white-space:nowrap;font-size:.81rem">
              <i class="bi bi-calendar3" style="color:var(--maroon);font-size:.72rem"></i> <?=fd($t['date_start'])?>
            ...
            <td style="font-size:.79rem;color:#666;white-space:nowrap">
              <?=ft($t['time_start'])?>
              <?php if($t['time_end']): ?><div style="color:#bbb;font-size:.74rem;margin-top:1px">→ <?=ft($t['time_end'])?></div><?php endif;?>
            </td>
            <td><span class="duration-chip"><i class="bi bi-clock" style="font-size:.65rem"></i> <?=fh($hours)?></span></td>
            ...
            <td style="font-size:.79rem;color:var(--muted)">
              ...
              <?php if(!empty($t['dept_name'])): ?><div style="font-size:.72rem;margin-top:1px"><?=htmlspecialchars($t['dept_name'])?></div><?php endif;?>
            </td>
          </tr>
          <?php endforeach;?>
        </tbody>
  ...
  <div class="print-footer">
    ...
    <div class="sig-row">
      ...
      <?php else: ?>
      <div class="sig-block">
        <span class="sig-name"><?=htmlspecialchars($_SESSION['username']??'')?></span>
...
<!-- ═══════════════════════════════════════════════════════
     VIEW SIGNED TICKET MODAL  (matches admin version)
════════════════════════════════════════════════════════ -->
...
document.querySelectorAll('.fptab').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        if (filterInput) filterInput.value = this.value || '';
    });
});
...
function triggerPrint() {
    ...
    setTimeout(() => {
        window.print();
        setTimeout(() => { document.title = orig; }, 600);
    }, 60);
}
