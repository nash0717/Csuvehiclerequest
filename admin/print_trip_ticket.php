...
/* ─── Office-based prefix ─── */
$officeName = strtolower(trim($s['office_name'] ?? ''));
if (str_contains($officeName, 'campus')) {
    $officePrefix = 'CAM-TRANSPO';
} elseif (str_contains($officeName, 'rde')) {
    $officePrefix = 'RDE-TRANSPO';
} else {
    $officePrefix = 'AUX-TRANSPO';
}

/* ─── Use stored ticket if valid, otherwise generate + save once ─── */
$storedTicket = $s['trip_ticket_no'] ?? '';

if (!empty($storedTicket) && preg_match('/\/\d{4}$/', $storedTicket)) {
    // Already stored and valid — never regenerate
    $tripTicketNo = $storedTicket;
} else {
    // Generate: rank within same office + same month/year only
    $seqStmt = $pdo->prepare("
        SELECT seq FROM (
            SELECT schedule_id,
                   RANK() OVER (
        ...
        WHERE schedule_id = ?
    ");
    $seqStmt->execute([$ticketMonth, $ticketYear, $s['office_id'], $schedule_id]);
    $seq = (int) $seqStmt->fetchColumn();

    $tripTicketNo = $officePrefix . '-' . $ticketMonth . '/' . $ticketYear . '/' . str_pad($seq, 4, '0', STR_PAD_LEFT);

    // Save permanently — one clean update, no duplicates
    $pdo->prepare("
        UPDATE schedules
        SET    trip_ticket_no = ?
        WHERE  schedule_id    = ?
