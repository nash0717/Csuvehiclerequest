...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if ($emailColumnExists && empty($email)) {
            ...
            $chkEmail = $pdo->prepare("SELECT user_id FROM users WHERE email = ? AND user_id != ?");
            $chkEmail->execute([$email, $userId]);
            if ($chkEmail->fetch()) $errors[] = 'That email address is already in use by another account.';
        }
...
/* ── Helpers ── */
function initials($u) {
    return strtoupper(substr($u['username'] ?? '?', 0, 1));
}
...
:root {
    ...
    --maroon-light: #fdf5f5; --maroon-mid: #f0e5e5;
    --gold: #c9a227; --text: #2a2a2a; --muted: #888;
    --radius: 14px; --shadow: 0 2px 16px rgba(128,0,0,.07);
}
body { background: #f5f0f0; font-family: 'Segoe UI', system-ui, sans-serif; color: var(--text); margin: 0; }
...
.sidebar .nav-link {
    color: rgba(255,255,255,.8); padding: .6rem 1.25rem;
...
.topbar-title { font-weight: 700; font-size: 1rem; color: var(--maroon); position: absolute; left: 45%; transform: translateX(-50%); }
.user-av-sm {
...
.section-header {
    padding: 1rem 1.25rem; border-bottom: 1px solid var(--maroon-mid);
    font-weight: 700; font-size: .9rem; color: var(--maroon);
    display: flex; align-items: center; justify-content: space-between;
...
.table thead th {
    ...
    letter-spacing: .04em; border-bottom: 2px solid var(--maroon-mid);
...
<div class="topbar">
    <div class="d-flex align-items-center gap-2">
        <button class="mobile-menu-btn" onclick="toggleSidebar()"
                style="background:none;border:none;cursor:pointer;padding:4px 8px;color:#800000;font-size:1.3rem;">
    ...
    <div class="d-flex align-items-center gap-2">
        ...
        <div>
            ...
            <div style="font-size:.72rem;color:var(--maroon)"><?= htmlspecialchars(ucfirst($user['role'] ?? 'Requestor')) ?></div>
        </div>
    </div>
</div>
            ...
            <table class="table mb-0">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        else echo '<span class="role-requestor"><i class="bi bi-person"></i>' . htmlspecialchars(ucfirst($r)) . '</span>';
                        ?>
                    </td>
        ...
        <div class="col-md-4">
            <div class="section-card">
                ...
                <div style="padding:.85rem 1.25rem;display:flex;flex-direction:column;gap:.6rem">
                    ...
                    <a href="my_trip.php" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-2" style="text-decoration:none">
                        <i class="bi bi-map"></i>My Trips
                    </a>
                    <a href="notification_requestor.php" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-2" style="text-decoration:none">
                        <i class="bi bi-bell"></i>Notifications
                    </a>
...
function updatePhoneFeedback(input, feedbackId) {
    var fb = document.getElementById(feedbackId), val = input.value;
    if (!fb) return;
    if (val.length === 0) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            if (ph) ph.setCustomValidity(/^09\d{9}$/.test(ph.value) ? '' : 'Must be 11 digits starting with 09.');
